...
class TrackerConfig
{
    ...
    protected function loadPorts()
    {
        ...
        foreach ($ports as $port) {
            ...
            if ($port->childPorts->isEmpty()) {
                continue;
            }

            $childAliases = $port->childPorts->pluck('name')->implode(',');
    ...
    protected function loadPort(TrackerPort $port)
    {
        $name = $port->name;

        $this->merge("$name.port", $port->port);
