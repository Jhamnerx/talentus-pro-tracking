                    ...
                    <tr>
                        ...
                        <td>
                            <span class="label label-sm label-{!! $item->active ? 'success' : 'danger' !!}">
                                {!! $item->active ? trans('validation.attributes.active') : trans('validation.attributes.inactive') !!}
                            </span>
                        </td>
                        <td>
                            {!! $item->email !!}
                        ...
                        <td>
                            {!! $item->hasExpiration() ? Formatter::time()->human($item->subscription_expiration) : trans('front.unlimited') !!}
                        </td>
                        <td>
                            ...
                            @if ($canEdit || $canLogin)
                                <div class="btn-group dropdown droparrow" data-position="fixed">
                                    ...
                                    <ul class="dropdown-menu">
                                        @if ($canEdit)
                                            <li>
                                                <a href="javascript:" data-modal="{!! $section !!}_edit"
                                        ...
                                        @if ($canLogin)
                                            <li>
                                                <a
                                                    href="{{ route('admin.clients.login_as_agree', $item->id) }}">{!! trans('front.login_as') !!}</a>
                                            </li>
                                        @endif
                                    </ul>
                                </div>
                            @endif
                            <i class="btn icon ico-arrow-down" type="button"
                                data-url="{{ route('admin.clients.get_devices', $item->id) }}" data-toggle="collapse"
                                data-target="#user-devices-{{ $item->id }}">
                            </i>
                        </td>
                    </tr>
                    <tr class="row-table-inner">
                        <td colspan="13" id="user-devices-{{ $item->id }}" aria-expanded="false" class="collapse">
                        </td>
                    </tr>
                @endforeach
            @else
