    ...
    <table class="table table-list sticky-header" id="history-table-content-table" data-toggle="multiCheckbox">
        <thead>
            <tr>
                ...
                @endif
                <th id="table-th-span-time" data-id="time" class="sorting {!! isset($sorting) && $sorting == 'desc' ? 'sorting_desc' : 'sorting_asc' !!}">
                    {!! trans('front.time') !!}</th>
        ...
        <tbody>
            @if (!empty($messages))
                @foreach ($messages as $message)
                    <tr data-position_id="{!! $message->id !!}" data-lat="{!! $message->latitude !!}"
                        data-lng="{!! $message->longitude !!}" data-speed="{!! $message->speed !!}"
                        data-altitude="{!! $message->altitude !!}" data-time="{!! $message->time !!}">
                        @if (Auth::User()->perm('history', 'remove'))
                            <td>
                                <div class="checkbox">
                        ...
                        @foreach ($sensors as $key => $sensor)
                            @if ($sensor['add_to_history'])
                                <td>{{ isset($message->sensors_value[$sensor['id']]) ? $message->sensors_value[$sensor['id']] : '-' }}
                                </td>
                            @endif
                        @endforeach
                        @foreach ($parameters as $param => $el)
                            <td>
                                @if (isset($message->other_array[$param]))
                                    {{ $message->other_array[$param] }}
                                @endif
                            </td>
                        @endforeach
