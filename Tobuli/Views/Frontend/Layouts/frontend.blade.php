...
<head>
    ...
    <style>
        @if (Appearance::getSetting('login_page_background_color'))
            body.sign-in-layout {
                background-color: {{ Appearance::getSetting('login_page_background_color') }};
            }
        @endif

        @if (Appearance::getSetting('login_page_text_color'))
            body.sign-in-layout .sign-in-text {
                color: {{ Appearance::getSetting('login_page_text_color') }};
            }
        @endif

        @if (Appearance::assetFileExists('background'))
            body.sign-in-layout {
                background-image: url({!! Appearance::getAssetFileUrl('background') !!} );
            }
        @endif

        @if (Appearance::getSetting('login_page_panel_background_color'))
            body.sign-in-layout .panel-background {
                background: {{ Appearance::getSetting('login_page_panel_background_color') }};
            }
        @endif

        @if (Appearance::getSetting('login_page_panel_transparency') != null)
            <?php $opacity = 1 - (int) Appearance::getSetting('login_page_panel_transparency') / 100; ?> body.sign-in-layout .panel-background {
                opacity: {{ $opacity }};
            }
        @endif
...
<body class="sign-in-layout"><!--<![endif]-->
