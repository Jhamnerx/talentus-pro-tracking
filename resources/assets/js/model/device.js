function Device(data) {
  ...
  _this.isVisible = function () {
    if (app.devices.forceVisible) return true;
  ...
  _this.setCheckOffline = function (timestamp, timeout) {
    ...
    if (options.online === "offline") return;

    var _timestamp = Math.max(options.timestamp, options.acktimestamp),
      _timeout = timestamp ? timeout - (timestamp - _timestamp) : timeout,
      _online = options.timestamp < options.acktimestamp ? "ack" : "offline";

    if (_online === "offline" && options.online === "blocked") return;
  ...
  _this.create = function (data) {
    ...
    if (typeof data.tail === "string") data.tail = JSON.parse(data.tail);
  ...
  _this.updateListItem = function (previous, current) {
    if (
      !(
        listDom && document.getElementsByTagName("body")[0].contains(listDom[0])
      )
    ) {
    ...
    if (previous.time_text != current.time_text && current.time_text)
      _this.dataDOMUpdate($('[data-device="time_text"]', listDom));
  ...
  _this.parseSensors = function (data) {
    ...
    if (typeof data.sensors !== "undefined") _sensors = data.sensors;

    if (typeof data.formatSensors !== "undefined")
      _sensors = data.formatSensors;

    if (typeof _sensors === "string") _sensors = JSON.parse(_sensors);

    $.each(_sensors, function (index, sensor) {
      if (typeof sensor.text !== "undefined") return;
  ...
  _this.getSensorByType = function (type) {
    ...
    if (!options.sensors) return null;
    ...
    switch (parameter) {
      ...
      case "streetview":
        ...
        _text =
          ...
          _course +
          ',0,0,0" target="_blank">';
        if (app.settings.streetViewFrame) {
          _text =
            '<iframe frameborder="0" style="width:290px;height:125px;" data-src="' +
            app.urls.streetViewFrame +
            "?size=" +
            data.size +
            "&amp;location=" +
            _lat +
      ...
      case "address":
        _text = "";
        _options =
          'data-lat="' + options.lat + '" data-lng="' + options.lng + '"';
        break;
      case "stoptime":
        _text = "";
  ...
  _this.dataSensorsDOM = function (sensors) {
    ...
    if (app.settings.showTotalDistance) {
      ...
      _row += "<tr>";
      _row +=
        '<td><i class="icon odometer"></i>' + window.lang.distance + "</td>";
      _row +=
        "<td>" +
        options.total_distance +
        " " +
        app.settings.units.distance.unit +
        "</td>";
      _row += "</tr>";
    ...
    if (options.distancia_hoy > 0) {
      _row = "";
      _row += "<tr>";
      _row += '<td><i class="icon odometer"></i>Distancia Hoy</td>';
      _row +=
        "<td>" +
        options.distancia_hoy +
        " " +
        app.settings.units.distance.unit +
        "</td>";
      _row += "</tr>";
    ...
    _row +=
      "<td>" + options.speed + " " + app.settings.units.speed.unit + "</td>";
    _row += "</tr>";
  ...
  _this.isLayerVisible = function () {
    ...
    if (!options.lat && !options.lng) return false;

    return app.settings.showDevice == true;
  };

  _this.getLayer = function () {
    if (!layer) layer = _this.updateLayer();
  ...
  _this.updateLayer = function () {
    ...
    if (options.icon.type === "arrow") {
      ...
      icon = '<img src="images/device_icons/estados/' + color + '.png" />';
    ...
    var divIcon = L.divIcon({
      ...
      className:
        "leaf-device-marker" +
        (app.devices.getSelected() === _this.id()
    ...
    if (!layer) {
      ...
      layer.setIcon(divIcon).setIconAngle(course);
    }
  ...
  _this.openPopup = function () {
    ...
    if (app.settings.showStreetView)
      nav +=
        '<li data-toggle="tooltip" data-placement="top" title="" role="presentation" data-original-title="Street View"><a href="#gps-device-street-view" aria-controls="gps-device-street-view" role="tab" data-toggle="tab"><i class="fa fa-road fa-1"></i></a></li>';

    nav +=
      '<li data-toggle="tooltip" data-placement="top" title="" role="presentation" class="active" data-original-title="Parameters"><a href="#gps-device-parameters" aria-controls="gps-device-parameters" role="tab" data-toggle="tab"><i class="fa fa-bars fa-1"></i></a></li>';
    nav +=
      '<li data-toggle="tooltip" data-placement="top" title="Close"><a href="javascript:" data-dismiss="popup"><i class="fa fa-times fa-1"></i></a></li>';
    nav += "</ul>";

    var navLargeView = "";
    navLargeView += '<ul class="nav nav-tabs nav-default" role="tablist">';
    navLargeView +=
      '<li><a href="#gps-device-parameters-view" aria-controls="gps-device-parameters-view" role="tab" data-toggle="tab"><i class="fa fa-compress"></i></a></li>';
    navLargeView +=
      '<li data-toggle="tooltip" data-placement="top" title="Close"><a href="javascript:" data-dismiss="popup"><i class="fa fa-times fa-1"></i></a></li>';
    navLargeView += "</ul>";
    ...
    streetViewHTML +=
      '<div class="buttons buttons-right"> <a href="#gps-device-street-view-large" class="btn" type="button" data-toggle="tab" aria-controls="gps-device-street-view-large">Enlarge <i class="fa fa-expand"></i></a> </div>';
    streetViewHTML += "</div>";
    ...
    parametersHTML +=
      "<tr><th>" +
      window.lang.stop_duration +
      ":</th><td>" +
      _this.dataDOM("stop_duration") +
      "</td></tr>";
    ...
    if (options.sensors) {
      $.each(options.sensors, function (index, sensor) {
        ...
        parametersHTML +=
          "<tr><th>" + sensor.name + ":</th><td>" + sensor.text + "</td></tr>";
      });
    }
    ...
    if (options.expiration_date)
      parametersHTML +=
        "<tr><th>" +
        window.lang.expiration_date +
        ":</th><td>" +
        _this.dataDOM("expiration_date") +
        "</td></tr>";

    parametersHTML += "</tbody></table>";
    parametersHTML +=
      '<div id="device-side-params" class="collapse"><table class="table table-condensed"><tbody>';
    parametersHTML +=
      "<tr><th>" +
      window.lang.position +
      ":</th><td>" +
      app.externalMapLink(options.lat, options.lng, _this.dataDOM("position")) +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.speed +
    ...
    parametersHTML +=
      "<tr><th>" +
      window.lang.altitude +
      ":</th><td>" +
      _this.dataDOM("altitude") +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.angle +
    ...
    parametersHTML +=
      ...
      window.lang.driver +
      ":</th><td>" +
      _this.dataDOM("driver.name") +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.model +
      ":</th><td>" +
      _this.dataDOM("device_model") +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.plate +
      ":</th><td>" +
      _this.dataDOM("plate_number") +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.protocol +
      ":</th><td>" +
      _this.dataDOM("protocol") +
      "</td></tr>";
    if (options.services) {
    ...
    parametersHTML +=
      '<div class="text-center"><i class="btn icon ico-options-h" data-toggle="collapse" data-target="#device-side-params"></i></div>';
    parametersHTML += "</div>";

    var html = "";
    html +=
      '<div class="popup-content" data-device-id="' +
      options.id +
      '"><div class="tab-content">';
    html +=
      '<div role="tabpanel" class="tab-pane active" id="gps-device-parameters-view">';
    html +=
      ...
      "</div></div>";
    html +=
      '   <div class="popup-body"><div class="tab-content">' +
      parametersHTML +
      streetViewHTML +
      "</div></div>";
    html += "</div>";
    html +=
      '<div role="tabpanel" class="tab-pane" id="gps-device-street-view-large">';
    html +=
      '   <div class="popup-header">' +
      navLargeView +
      '<div class="popup-title">' +
      window.lang.streetview +
      "</div></div>";
  ...
  _this.updatePopup = function () {
    ...
    popup.setLatLng(_this.getLatLng()).setContent(_content).update();
  ...
  _this.refreshWitgets = function (isFull) {
    ...
    if (!app.widgets || !app.widgets.length) return;

    if (app.widgets.attr("data-device-id") != options.id) return;
  ...
  _this.updateWitgets = function () {
    ...
    app.widgets
      .find('[data-modal="services_create"]')
      .attr("data-url", app.urls.devicesServiceCreate + _this.id());
    app.widgets
      .find('[data-modal="sensors_create"]')
      .attr("data-url", app.urls.devicesSensorCreate + _this.id());
    app.widgets
      .find('[data-modal="lock_history"]')
      .data("url", app.urls.lockHistory + _this.id());
    app.widgets
      .find('[data-modal="unlock_lock"]')
      .data("url", app.urls.unlockLock + _this.id());
    app.widgets
      .find('[data-modal="services"]')
    ...
    if ($lockingWidget.length) {
      $.ajax({
        ...
        success: function (response) {
          $lockingWidget.find(".status-message").text(response.message);
  ...
  _this.getTailPoints = function () {
    ...
    if (latlng) tailPoints.push(latlng);
  ...
  _this.addTail = function () {
    ...
    if (!app.settings.showDevice) return;

    if (!app.settings.showTail) return;

    tail = L.polyline(_this.getTailPoints(), {
      color: options.tail_color,
      className: "leaf-device-tail",
    });
  ...
  _this.removeInaccuracy = function () {
    if (inaccuracy) inaccuracy.remove();
  ...
  _this.calcStopDuration = function () {
    var _offset =
        parseInt(moment.utc().valueOf() / 1000) - options.timestamp_offset,
      _timestamp = options.timestamp ? parseInt(options.timestamp) : 0,
      _moved_at = options.moved_timestamp
        ? parseInt(options.moved_timestamp)
        : 0,
      _stop = options.stop_duration_sec + _offset,
      _time = 0;

    if (options.online == "online") return secondsToTime(_time);

    if (!_timestamp) return "-";

    if (!_moved_at) return "12+" + window.lang.short_h;

    if (_stop < 60) return secondsToTime(_time);
  ...
  _this.updateGprsCommandWidget = function () {
    ...
    $.ajax({
      ...
      complete: function () {
        ...
        app.widgets
          .find("#widget-gprs-command")
          .find(".btn.send-command")
          .off("click")
          .on("click", function () {
            var container = $(this).closest("#widget-gprs-command");
            var deviceId = container.find('input[name="device_id"]').val();
