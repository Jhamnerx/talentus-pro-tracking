var app = {
    ...
    initStreetView: function() {
        $.ajax({
            type: 'GET',
            dataType: "json",
            url: app.urls.streetViewEnable,
            success: function(res){
                var status = false;
                if (res){
                    status = res.enable;
                }
                app.settings.streetViewFrame = status;
            },
            error: function(jqXHR, textStatus, errorThrown) {
                app.settings.streetViewFrame = false;
            }
        });
    },
...
function initComponents( container ) {
    ...
    $('iframe[data-src]:visible', $container).each( function() {
        var $url = $(this);
        $.ajax({
            type: 'GET',
            async: true,
            url: $url.attr('data-src'),
            success: function (res) {
                $url.attr('src', res.urlFrame).removeAttr('data-src');
            },
            error: function(jqXHR, textStatus, errorThrown) {
                $url.attr('src', $url.attr('data-streetview')).removeAttr('data-src');
            }
        });
    });
